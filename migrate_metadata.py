...
V2_GALLERY_HEADER = [
    ...
    "stripe_extent", "stripe_thickness", "arm_color", "arm_thickness", "central_disc_color",
...
def migrate_encounter_dates_main() -> int:
    """
    Entry point for migrating encounter dates from folder names to encounter_dates.csv.
    
    Run with: python migrate_metadata.py --encounter-dates [--dry-run]
    """
    parser = argparse.ArgumentParser(
        description="Migrate encounter dates from folder names to encounter_dates.csv"
    )
    parser.add_argument("--dry-run", action="store_true", help="Preview changes without writing")
    args = parser.parse_args()
    
    print("=" * 60)
    print("Encounter Dates Migration")
    print("=" * 60)
    print()
    print("This will scan all encounter folders and populate encounter_dates.csv")
    print("with dates parsed from folder names (MM_DD_YY format).")
    print()
    
    try:
        from src.data.encounter_info import migrate_encounter_dates_from_folders
        stats = migrate_encounter_dates_from_folders(dry_run=args.dry_run)
    except ImportError as e:
        print(f"[ERROR] Failed to import migration function: {e}")
        return 1
    
    print("-" * 40)
    print("Results")
    print("-" * 40)
    print(f"  Gallery encounters added:  {stats['gallery_added']}")
    print(f"  Queries encounters added:  {stats['queries_added']}")
    print(f"  Entries skipped (existing or unparseable): {stats['skipped']}")
    
    if args.dry_run:
        print("\n[DRY-RUN] No files were modified. Run without --dry-run to apply changes.")
    else:
        print(f"\n[DONE] Added {stats['gallery_added'] + stats['queries_added']} entries to encounter_dates.csv")
...
if __name__ == "__main__":
    import sys
    if "--encounter-dates" in sys.argv:
        sys.argv.remove("--encounter-dates")
        exit(migrate_encounter_dates_main())
    else:
        exit(main())
