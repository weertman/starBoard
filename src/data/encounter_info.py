...
def migrate_encounter_dates_from_folders(dry_run: bool = False) -> Dict[str, int]:
    """
    Migrate encounter dates from folder names to encounter_dates.csv.
    
    This is a one-time migration utility for backward compatibility.
    It scans all encounter folders and populates encounter_dates.csv
    with dates parsed from the folder names (MM_DD_YY format).
    
    Only adds entries that don't already exist in the CSV.
    
    Args:
        dry_run: If True, don't write to CSV, just report what would be done.
        
    Returns:
        Dict with stats: {"gallery_added": N, "queries_added": N, "skipped": N}
    """
    stats = {"gallery_added": 0, "queries_added": 0, "skipped": 0}
    
    # Load existing entries
    existing = _load_encounter_dates()
            ...
            for id_dir in root.iterdir():
                ...
                for enc_dir in id_dir.iterdir():
                    if not enc_dir.is_dir():
                        continue
                    enc_name = enc_dir.name
                    
                    # Check if already exists in CSV
                    key = (target, id_str, enc_name)
                    if key in existing:
                        stats["skipped"] += 1
                        continue
                    
                    # Parse date from folder name
                    d = _parse_mmddyy(enc_name)
                    if d is None:
                        stats["skipped"] += 1
                        continue
                    
                    # Add to CSV (unless dry run)
                    if not dry_run:
                        set_encounter_date(target, id_str, enc_name, d)
