...
_STAR_DATE_RE = _re.compile(r"^(\d{1,2})_(\d{1,2})_(\d{4})")


def discover_star_dataset(root_dir):
    """Scan a star_dataset_raw directory with 3-level structure.

    Structure: root / data_group / individual_id / M_D_YYYY_description / images

    Returns:
        list of (data_group, individual_id, encounters)
        where encounters = [(folder_name, parsed_date, [image_paths])]
    """
    root = Path(root_dir)
    results = []
    ...
    for group_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        ...
        for id_dir in sorted(p for p in group_dir.iterdir() if p.is_dir()):
            individual_id = id_dir.name
            encounters = []
            for enc_dir in sorted(p for p in id_dir.iterdir() if p.is_dir()):
                m = _STAR_DATE_RE.match(enc_dir.name)
                if not m:
                ...
                try:
                    ...
                    log.warning("Invalid date in folder name: %s", enc_dir.name)
                    continue
                images = sorted(
                ...
                if images:
                    encounters.append((enc_dir.name, parsed_date, images))
            if encounters:
                results.append((data_group, individual_id, encounters))

    log.info("discover_star_dataset: found %d id entries under %s", len(results), root)
    return results