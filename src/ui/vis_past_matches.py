...
def _group_queries_by_outing() -> Dict[str, List[str]]:
    """
    Group all query IDs by their outing.
    
    Returns:
        {outing_name: [query_id1, query_id2, ...]}
    """
    outings: Dict[str, List[str]] = defaultdict(list)
    query_ids = list_ids("Queries")
    ...
    for qid in query_ids:
        ...
        if parsed is None:
            continue
        date_str, outing_name = parsed
        outings[outing_name].append(qid)
    
    # Sort queries within each outing
    for outing_name in outings:
        outings[outing_name].sort()
    
    return dict(outings)


def _get_outing_date(outing_name: str) -> str:
    """Extract date string from outing name for sorting."""
    # Outing names are like "9_10_2025_release_day_dive_one"
    # Try to parse the date portion
    parts = outing_name.split("_")
    if len(parts) >= 3:
        try:
...
class QueryGridDialog(QDialog):
    ...
    def _populate_filter_table(self):
        ...
        sorted_outings = sorted(
            ...
            key=lambda x: (_get_outing_date(x[0]), x[0])
        )
        
        self._filter_table.setRowCount(len(sorted_outings))
        
        for row, (outing_name, query_ids) in enumerate(sorted_outings):
            # Checkbox
            chk = QTableWidgetItem()
            chk.setFlags(Qt.ItemIsUserCheckable | Qt.ItemIsEnabled)
            chk.setCheckState(Qt.Unchecked if outing_name in self._excluded_outings else Qt.Checked)
    ...
    def _get_filtered_outings(self) -> List[Tuple[str, List[str]]]:
        ...
        return sorted(
            [(k, v) for k, v in self._outings_data.items() if k not in self._excluded_outings],
            key=lambda x: (_get_outing_date(x[0]), x[0])
        )
