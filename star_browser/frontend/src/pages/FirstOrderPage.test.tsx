...
describe('FirstOrderPage query selector', () => {
  ...
  it('offers MegaStar as a preset and shows its numeric field in the Top-K breakdown', async () => {
    ...
    expect(galleryFilters).not.toHaveStyle({ maxHeight: '320px' })
    expect(galleryFilters).not.toHaveStyle({ overflow: 'auto' })
    ...
    await user.selectOptions(locationSelect, 'Eagle Point')
    expect(galleryFilters).toHaveTextContent('Selected location: Eagle Point')
